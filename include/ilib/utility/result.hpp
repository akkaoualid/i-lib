...
struct Result {
    ...
    constexpr void swap(Result<R, E>&& other) noexcept {
        using std::swap;
        if(is_ok() && other.is_ok()) {
            swap(m_ok_ins, other.m_ok_ins);
        } else if(is_err() && other.is_err()) {
            swap(m_err_ins, other.m_err_ins);
        }
    ...
    constexpr Option<T> ok() const noexcept {
        if (is_ok()) {
            return ilib::Some(m_ok_ins.value);
        } else if (is_err()) {
            return ilib::None;
        }
    }
    ...
    constexpr auto as_ref() const noexcept
        -> Result<std::reference_wrapper<T>, std::reference_wrapper<U>> {
        if (is_ok())
            return {Ok<std::reference_wrapper<T>>{std::ref(m_ok_ins.value)}};
        else
            return {Err<std::reference_wrapper<U>>{
                std::ref(m_err_ins.get_value)()}};
    }
    constexpr auto as_cref() const noexcept
        ...
        if (is_ok())
            return {
                Ok<std::reference_wrapper<const T>>{std::cref(m_ok_ins.value)}};
        else
            return {Err<std::reference_wrapper<const U>>{
                std::cref(m_err_ins.value)}};
    }

    template <class F>
    constexpr auto map(F&& func) const
        noexcept(std::is_nothrow_invocable_r_v<
                 Result<std::invoke_result_t<F, T>, U>, F, T>)
            -> Result<std::invoke_result_t<F, T>, U> {
        if (is_ok()) {
            return {Ok<std::invoke_result_t<F, T>>(
                std::forward<F>(func)(ok().unwrap()))};
        } else {
            return m_err_ins;
        }
    }
    ...
    constexpr auto map_err(F&& func) const
        noexcept(std::is_nothrow_invocable_r_v<
                 Result<T, std::invoke_result_t<F, U>>, F, U>)
            -> Result<T, std::invoke_result_t<F, U>> {
        if (is_ok()) {
            return {Err<std::invoke_result_t<F, U>>(
                std::forward<F>(func)(m_err_ins.value))};
        } else {
            return m_err_ins;
        }
    ...
    constexpr auto and_then(F&& func) const noexcept(
        std::is_nothrow_invocable_r_v<Result<T, U>, F, T>)
        -> Result<T, U> requires(std::is_invocable_r_v<Result<T, U>, F, T>) {
        if (is_ok())
    ...
    constexpr auto or_else(F&& func) const
        noexcept(std::is_nothrow_invocable_v<F, U>)
            -> Result<std::invoke_result_t<F, U>, U> {
        if (is_ok())
            return *this;
        else
            return std::forward<F>(func)(m_err_ins.value);
    }
